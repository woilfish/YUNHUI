...
public class StringUtil {
     ...
     * 列表获取指定时间距今的文字描述。
     */
    public static String getDateCircleDescription(Context context, long times) {
        ...
        if (times <= 0) {
            ...
            timeIntoFormat = timePassed / ONE_MINUTE;
            updateAtValue = timeIntoFormat + "分钟之前";
        } else if (timePassed < ONE_DAY) {
            timeIntoFormat = timePassed / ONE_HOUR;
            updateAtValue = timeIntoFormat + "小时之前";
        } else if (timePassed < ONE_MONTH) {
            timeIntoFormat = timePassed / ONE_DAY;
            updateAtValue = timeIntoFormat + "天之前";
        } else if (timePassed < ONE_YEAR) {
            timeIntoFormat = timePassed / ONE_MONTH;
            updateAtValue = timeIntoFormat + "个月之前";
        } else {
            timeIntoFormat = timePassed / ONE_YEAR;
            updateAtValue = timeIntoFormat + "年之前";
        }
        return updateAtValue;
    }
     ...
     * @return
     */
    public static double formatDistance(double data) {
        BigDecimal bg = BigDecimal.valueOf(data);
        return bg.setScale(1, BigDecimal.ROUND_DOWN).doubleValue();
    }
}
